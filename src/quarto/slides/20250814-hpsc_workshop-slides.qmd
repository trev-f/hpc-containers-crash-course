...
* Open OnDemand: <https://login.isaac.utk.edu> -> Files -> Open in Terminal
...
proj_dir="${SCRATCHDIR}/hpsc-using-containers"
...
## Fetch toy data {visibility="hidden"}
...
1. Make you data directory
2. Copy toy data files into your directory
...
# copy BAM and index
cp /lustre/isaac24/proj/UTK0411/tfreem10/hpc-containers-crash-course/data/alignments/* "${alignments_dir}"
```

# Run software in Apptainer containers

## Get started with Apptainer CLI
...
## Terminology Note: Images and Containers {.smaller}
...
::: {.column width="50%"}
![Image](images/blueprint.jpeg)
:::

::: {.column width="50%"}
![Container](images/house.jpg)
:::
...
apptainer pull samtools_1-21.sif "${samtools_img_uri}"    # <1>
```

1. It's typical to write images to file with `.sif` extension.

## 3 ways to run container from image {.smaller}
...
::: {.callout-note title="Isolation"}
This is what we mean we say containers provide isolated environments.
:::

## `apptainer shell` -- Files available in the Samtools container {.smaller}
...
ls /lustre/isaac24/proj
```

. . .

::: {.callout-important title="Bind mounts"}
These directories are mounted when we run the container -- they are not part of the image.
Bind mounts are set in the configuration or during the container run command.
The ISAAC-NG sys admins have configured these directories to be mounted by default.
Other systems will have different configurations, and may not be as user friendly by default.
Thanks, sys admins!
:::
...
::: {.callout-important title="`apptainer run`"}
Running this command is completely dependent on the runscript.
Since we have no control over the runscript of a container we did not create, I tend to not recommend relying on `apptainer run`.
:::
...
`stdout`/`stdin`/`stderr` goes back and forth between the container and the host OS.
In the command above, the filtered BAM stream from the container is piped into the `samtools` command executed on *the host OS*.
We don't have the `samtools` command available on the host OS, thus the `command not found` error.
:::
...
Many URI protocols are listed as valid `<container>` options.
...
Since `samtools` is a function and not an executable file here, there will be situations in which it doesn't behave exactly like running the `samtools` command.
There may also be scenarios where passing arguments to the `samtools` command in the container through the `samtools` function doesn't behave as expected.

Long story short: beware of edge cases.
This abstraction is best for experienced shell script writers and very straightforward use cases.
Don't trade working code for pretty code!
:::
...
* Execute directly from remote URI
...
* "Load" software with function
  ...
  * May be edge cases with unexpected behavior
...
::: {.callout-note title="Cache Docker images"}
Docker images are also cached by Apptainer.
Rerun the command from above and it should run much faster.
:::
...
Takes a while to build image file. Watch for changes to file with `wait tail -n 20 .cache/sbatch_logs/<job ID>_-_rstudio_server.err`
